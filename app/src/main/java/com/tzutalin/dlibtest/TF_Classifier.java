...
public class TF_Classifier implements Classifier {
    ...
    public static TF_Classifier create(AssetManager assetManager, String name,
                                              String modelPath, String labelFile, int inputSize,int inputFrames, String inputName, String outputName,
                                              boolean feedKeepProb) throws IOException {
        //intialize a classifier
        TF_Classifier c = new TF_Classifier();
    ...
    @Override
    public float[] recognize(final float[] pixels) {

        //using the interface
        //give it the input name, raw pixels from the drawing,
        //input size
        tfHelper.feed(inputName, pixels,new long[]{1,25,20});
        //tfHelper.feed(inputName, pixels, new long[]{25,20,1}, inputFrames, inputSize,1);
